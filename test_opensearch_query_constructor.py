...
def test_api_service_errors_specific_date(constructor):
    ...
    assert any(tr.get("gte") == "2025-04-11T00:00:00Z" for tr in time_ranges)
    assert any(tr.get("lte") == "2025-04-11T23:59:59Z" for tr in time_ranges)
...
def test_mindbox_upload_errors_date_range(constructor):
    ...
    time_ranges = [f["range"]["time"] for f in filters if "range" in f and "time" in f["range"]]
    assert any("time_zone" in tr and tr["time_zone"] == "+03:00" for tr in time_ranges)
    assert any(tr.get("gte") == "2025-03-20T10:00:00Z" for tr in time_ranges)
    assert any(tr.get("lte") == "2025-03-21T10:00:00Z" for tr in time_ranges)
...
def test_errors_in_prod_date_range(constructor):
    ...
    time_ranges = [f["range"]["time"] for f in filters if "range" in f and "time" in f["range"]]
    assert any("time_zone" in tr and tr["time_zone"] == "+03:00" for tr in time_ranges)
    assert any(tr.get("gte") == "2025-03-20T00:00:00Z" for tr in time_ranges)
    assert any(tr.get("lte") == "2025-03-21T23:59:59Z" for tr in time_ranges)
...
def test_order_psv_745559_time_range(constructor):
    ...
    assert any("time_zone" in tr and tr["time_zone"] == "+03:00" for tr in time_ranges)
    assert any(tr.get("gte") == "2025-03-20T10:00:00Z" for tr in time_ranges)
    assert any(tr.get("lte") == "2025-03-21T11:35:56Z" for tr in time_ranges)
